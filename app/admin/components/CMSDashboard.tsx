...
const CMS_TABS: { id: CMSTab; label: string }[] = [
...
export default function CMSDashboard({ onLogout, isDark, onToggleTheme }: Props) {
  ...
  async function handleDiscard(tab: CMSTab) {
    if (!config) return;
  ...
  const navSections: { id: Section; label: string; icon: React.ReactNode }[] = [
    { id: "cms",       label: "CMS",       icon: <LayoutGrid size={15} /> },
            ...
            {section === "cms" && (
              <>
                ...
                {pendingChanges.length > 0 && (
                  <div className="mb-5 px-4 py-4 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-xs text-amber-600 dark:text-amber-400 hidden sm:block">
                        Preview then Publish to go live
                      </span>
                    </div>
                    <ul className="flex flex-wrap gap-2">
                      {pendingChanges.map((change) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleDiscard(change.tab as CMSTab)}
                ...
                {!loading && config && (
                  <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                        ...
                        {CMS_TABS.map((tab) => (
                          <li key={tab.id} className="flex-shrink-0">
                            <button
                              ...
                              className={`relative px-3 py-3.5 text-xs font-medium transition-colors whitespace-nowrap ${
                                activeTab === tab.id
                            ...
                            >
                              ...
                              {pendingChanges.some((c) => c.tab === tab.id) && (
                                <span className="absolute top-2.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-amber-400" />
                              )}
                            </button>
                          </li>
                        ))}
                    ...
                    <div className="p-6 sm:p-8">
                      {activeTab === "branding" && (
                        <BrandingTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "hero" && (
                        <HeroTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "homepage" && (
                        <HomepageTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "company" && (
                        <CompanyTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "reviews" && (
                        <ReviewsTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "gallery" && (
                        <GalleryTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "contact" && (
                        <ContactTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "features" && (
                        <FeaturesTab config={config} onSave={handleSave} saving={saving} />
                      )}
                      {activeTab === "serviceAreas" && (
                        <ServiceAreasTab config={config} onSave={handleSave} saving={saving} />
                      )}
                    </div>
                  </div>
                )}
