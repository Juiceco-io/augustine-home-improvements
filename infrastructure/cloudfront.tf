...
resource "aws_cloudfront_response_headers_policy" "security_headers" {
  ...
  security_headers_config {
    ...
    # Note: adjust CSP to allow Google Analytics and Google Fonts if needed.
    content_security_policy {
      content_security_policy = "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.googletagmanager.com https://www.google-analytics.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https://www.google-analytics.com; frame-ancestors 'none'; connect-src 'self' https://www.google-analytics.com"
      override                = true
    }
  }
...
resource "aws_cloudfront_distribution" "site" {
  ...
  # 403/404 from S3 → show our 404 page
